...
static void adios_completed_request(struct request *rq) {
	...
	timer_reduce(&ad->update_timer, jiffies + msecs_to_jiffies(1000)); // ponytail: was 100ms, 10x fewer timer wakes
	}
}
...
static bool adios_has_work(struct blk_mq_hw_ctx *hctx) {
	...
	return atomic_read(&ad->state) & 0x3F; // ponytail: ignore barrier-in-progress bit (6), only check queue bits
}
